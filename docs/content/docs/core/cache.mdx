---
title: Cache & Redis
description: Redis-backed cache store with connection pooling, circuit breaker protection, and a provider-agnostic Store interface.
---

Mannaiah caches are accessed through the `cache.Store` interface, which is implemented by the
`redis` package. This separation means cache consumers are not coupled to Redis: any backend
that satisfies the interface can be substituted.
