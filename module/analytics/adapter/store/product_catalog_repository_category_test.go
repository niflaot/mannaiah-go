...
func TestResolveProductIDsSupportsCategoryIDSlugAndName(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			got, err := repo.resolveProductIDs(context.Background(), []string{"tier-1"}, "any", nil, tt.categoryID, nil, nil, 10)
			if err != nil {
...
func TestResolveProductIDsCategoryFallbackToRawReference(t *testing.T) {
	...
	got, err := repo.resolveProductIDs(context.Background(), []string{"tier-1"}, "any", nil, "legacy-cat", nil, nil, 10)
	if err != nil {
		t.Fatalf("resolveProductIDs() error = %v", err)
	}
