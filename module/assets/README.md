...
- Methods: `assets.New(db, storage, publishers...)`, `assets.NewWithConfig(cfg, db, storage, logger, publishers...)`, `(*assets.Module).ConfigureScheduler`, `(*assets.Module).Start`, `(*assets.Module).Stop`, `(*assets.Module).RegisterRoutes`, `(*assets.Module).SetAuthorizer`, `(*assets.Module).Load`, `(*assets.Module).Service`, `assets.OpenAPISpec()`
