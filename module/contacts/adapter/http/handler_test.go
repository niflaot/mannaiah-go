...
// TestHandlerCircleOptInEndpoint verifies POST /contacts/optin behavior.
func TestHandlerCircleOptInEndpoint(t *testing.T) {
	fixedNow := time.Date(2026, time.March, 13, 18, 5, 22, 0, time.UTC)
	handler := newHandlerForTest(t, serviceMock{
		createFn: func(ctx context.Context, command application.CreateCommand) (*domain.Contact, error) { return nil, nil },
		getFn:    func(ctx context.Context, id string) (*domain.Contact, error) { return nil, nil },
		listFn: func(ctx context.Context, query port.ListQuery) (*application.ListResult, error) {
			if query.Email != "consent@example.com" {
				t.Fatalf("query.Email = %q, want %q", query.Email, "consent@example.com")
			}
		...
		updateFn: func(ctx context.Context, id string, command application.UpdateCommand) (*domain.Contact, error) {
			if id != "c-1" {
				t.Fatalf("id = %q, want %q", id, "c-1")
			}
			if command.Metadata == nil {
				t.Fatalf("expected metadata payload")
			}
			if (*command.Metadata)["flock_checker_circle_optin"] != "yes" {
				t.Fatalf("flock_checker_circle_optin = %q, want %q", (*command.Metadata)["flock_checker_circle_optin"], "yes")
			}
	...
	request, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optin", bytes.NewBufferString(`{"email":"consent@example.com"}`))
...
// TestHandlerCircleOptOutEndpoint verifies POST /contacts/optout behavior.
func TestHandlerCircleOptOutEndpoint(t *testing.T) {
	fixedNow := time.Date(2026, time.March, 13, 18, 5, 22, 0, time.UTC)
	handler := newHandlerForTest(t, serviceMock{
		createFn: func(ctx context.Context, command application.CreateCommand) (*domain.Contact, error) { return nil, nil },
		getFn:    func(ctx context.Context, id string) (*domain.Contact, error) { return nil, nil },
	...
	request, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optout", bytes.NewBufferString(`{"email":"consent@example.com"}`))
	request.Header.Set("Content-Type", "application/json")
	response := runRequest(t, server, request)
	if response.StatusCode != stdhttp.StatusOK {
		t.Fatalf("status = %d, want %d", response.StatusCode, stdhttp.StatusOK)
...
// TestHandlerCircleOptInEndpointNotFound verifies missing contact behavior.
func TestHandlerCircleOptInEndpointNotFound(t *testing.T) {
	handler := newHandlerForTest(t, serviceMock{
		createFn: func(ctx context.Context, command application.CreateCommand) (*domain.Contact, error) { return nil, nil },
		getFn:    func(ctx context.Context, id string) (*domain.Contact, error) { return nil, nil },
		listFn: func(ctx context.Context, query port.ListQuery) (*application.ListResult, error) {
			return &application.ListResult{Data: []domain.Contact{}}, nil
		},
		updateFn: func(ctx context.Context, id string, command application.UpdateCommand) (*domain.Contact, error) {
			return nil, nil
		},
	...
	request, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optin", bytes.NewBufferString(`{"email":"missing@example.com"}`))
	request.Header.Set("Content-Type", "application/json")
	response := runRequest(t, server, request)
	if response.StatusCode != stdhttp.StatusNotFound {
		t.Fatalf("status = %d, want %d", response.StatusCode, stdhttp.StatusNotFound)
	}
}

// TestHandlerCircleOptInAuthEnforcement verifies contacts:manage authorization behavior.
func TestHandlerCircleOptInAuthEnforcement(t *testing.T) {
	unauthorizedError := errors.New("unauthorized")
	forbiddenError := errors.New("forbidden")
	...
	handler := newHandlerForTest(t, serviceMock{
		...
		getFn:    func(ctx context.Context, id string) (*domain.Contact, error) { return nil, nil },
		listFn: func(ctx context.Context, query port.ListQuery) (*application.ListResult, error) {
			return &application.ListResult{Data: []domain.Contact{{ID: "c-1", Email: "consent@example.com"}}}, nil
		},
		updateFn: func(ctx context.Context, id string, command application.UpdateCommand) (*domain.Contact, error) {
			return &domain.Contact{ID: id, Email: "consent@example.com"}, nil
		},
		deleteFn: func(ctx context.Context, id string) error { return nil },
	}, authorizerMock{
		requireFn: func(ctx context.Context, authorizationHeader string, requiredPermissions ...string) error {
			...
			if len(requiredPermissions) != 1 || requiredPermissions[0] != "contacts:manage" {
				t.Fatalf("requiredPermissions = %#v, want contacts:manage", requiredPermissions)
			}
			return nil
		},
		isUnauthorizedFn: func(err error) bool {
	...
	unauthorizedReq, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optin", bytes.NewBufferString(`{"email":"consent@example.com"}`))
	...
	forbiddenReq, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optin", bytes.NewBufferString(`{"email":"consent@example.com"}`))
	...
	allowedReq, _ := stdhttp.NewRequest(stdhttp.MethodPost, "/contacts/optin", bytes.NewBufferString(`{"email":"consent@example.com"}`))
