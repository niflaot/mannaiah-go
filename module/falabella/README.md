...
## Sync Feed Task Semantics
- Product sync responses include `feeds[]` entries with `step` and `task`.
- `task=data` identifies catalog data feeds (`step=product`) even when Falabella action is `ProductCreate` or `ProductUpdate`.
- `task=image` identifies image feeds (`step=image`) generated by `SyncProductImages`.
