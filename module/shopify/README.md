...
Contact synchronization now persists `shopify_sync_links`, stitches inbound-created links before outbound fan-out, and pushes mainstream contact changes back to Shopify without re-emitting equivalent webhook echoes. Manual contact sync reconciles both directions: Shopify customers are imported first, then local contacts are linked/created in Shopify with email/link deduplication.
Order synchronization uses the same link table: linked orders receive status/tag updates, while unlinked Mannaiah orders create Shopify orders assigned to the correct Shopify customer. Manual order sync reconciles both directions: Shopify orders are imported first, then local Shopify-realm orders pre-sync their contacts and create/update Shopify orders without recursive echo loops.
