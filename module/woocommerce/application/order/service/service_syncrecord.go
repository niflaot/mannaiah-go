...
func (s *OrderSyncService) startSyncRunRecord(ctx context.Context, trigger string) string {
	if s == nil || s.syncRecorder == nil {
		return ""
	}

	runID, err := s.syncRecorder.StartRun(ctx, "woocommerce.orders", trigger)
	if err != nil {
		s.logger.Warn("woocommerce orders sync recorder start failed", zap.Error(err))
		return ""
	}
...
func (s *OrderSyncService) finishSyncRunRecord(ctx context.Context, runID string, summary *SyncSummary, syncErr error) {
	...
	if syncErr == nil {
		if err := s.syncRecorder.CompleteRun(ctx, runID, summary.Processed, succeeded, summary.Failed, summary.Skipped); err != nil {
			s.logger.Warn("woocommerce orders sync recorder complete failed", zap.Error(err))
		}
		return
	}
...
func classifySyncError(err error) string {
	...
	if errors.Is(err, ErrInvalidOrderID) || errors.Is(err, ErrOrderNotFound) {
		return "validation"
	}
